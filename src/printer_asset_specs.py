...
from src import app_shell, assets
...
def render_printer_asset_specs() -> None:
    ...
    st.caption("Completa una sola vez los datos que utilizarán automáticamente costeo, mantenimiento y producción.")
    ...
    with st.form("printer_asset_spec_form"):
        ...
        a, b, c, d = st.columns(4)
        ink_c = a.number_input("Botella C ($)", min_value=0.0, value=float(current.get("ink_c", 19.0)))
        ink_m = b.number_input("Botella M ($)", min_value=0.0, value=float(current.get("ink_m", 19.0)))
        ink_y = c.number_input("Botella Y ($)", min_value=0.0, value=float(current.get("ink_y", 19.0)))
        ink_k = d.number_input("Botella K ($)", min_value=0.0, value=float(current.get("ink_k", 19.0)))
        a, b, c = st.columns(3)
    ...
    if submitted:
        ...
        st.success("Ficha técnica guardada y disponible para el costeo automático.")
        st.rerun()
...
def activate_printer_asset_specs() -> None:
    ...
    try:
        ...
        top_navigation_app.DESCRIPTIONS[name] = "Rendimientos, tintas, cabezales, velocidad y consumo eléctrico."
