...
def render_inventory_stock_with_metadata_editor(rows: list[dict]) -> None:
    ...
    with st.form("inventory_metadata_safe_form"):
        a, b, c = st.columns(3)
        location = a.text_input("Ubicación", value=str(item.get("location") or ""))
        lot = b.text_input("Lote", value=str(item.get("lot") or ""))
        expiry = c.date_input("Vencimiento", value=expiry_default)
        reason = st.text_input("Motivo / responsable obligatorio")
        confirm = st.checkbox("Confirmo que solo actualizaré datos logísticos del artículo")
        submit = st.form_submit_button("Guardar cambio auditado", type="primary", use_container_width=True)
