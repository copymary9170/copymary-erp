"""Compatibilidad para excluir pagos revertidos y registrar controles globales."""
...
def _receivable_payments_for(sale_id: str, payments: list[dict]) -> list[dict]:
    return [item for item in payments if str(item.get("sale_id", "")) == sale_id and not item.get("reversed")]
...
def _sale_paid(sale: dict, payments: list[dict]) -> float:
    ...
    explicit = sum(float(item.get("amount", 0.0)) for item in payments if str(item.get("sale_id", "")) == sale_id and not item.get("reversed"))
    if explicit > 0:
...
def _purchase_paid(purchase: dict, payments: list[dict]) -> float:
    ...
    explicit = sum(float(item.get("amount", 0.0)) for item in payments if str(item.get("purchase_id", "")) == purchase_id and not item.get("reversed"))
    if explicit > 0:
        return min(explicit, total)
...
def _linked_references(reference_id: str, payments: list[dict], link_key: str) -> set[str]:
    ...
    references.update(str(item.get("payment_id", "")) for item in payments if str(item.get(link_key, "")) == reference_id and item.get("payment_id") and not item.get("reversed"))
    return references
