...
def _migrate_core_entities_v16(connection: Any) -> None:
    """Crea el almacén compatible de entidades núcleo sin tocar datos existentes."""
    connection.executescript("""
...
def _migrate_auth_schema_v17(connection: Any) -> None:
    """Alinea ``app_roles`` y ``app_users`` con el contrato real de ``auth.py``.

    Bases antiguas crearon ``app_roles`` con la columna ``role_name`` y sin
    ``created_at_utc``, y ``app_users`` sin la columna ``status``. Eso rompía
    ``auth.py`` ("no such column: name" / "no column named status"), dejando
    inoperantes usuarios, roles y permisos. Renombra ``role_name`` → ``name``,
    agrega ``created_at_utc`` en ``app_roles`` y ``status`` en ``app_users`` de
    forma idempotente, conservando los datos existentes.
    """
    ...
    _ensure_columns(connection, "app_permissions", {
        "permission_id": "TEXT",
...
def _migrate_payroll_schema_v18(connection: Any) -> None:
    """Alinea las tablas de nómina/RR. HH. con el contrato real de ``payroll.py``.

    La migración v13 creó ``employee_time_off`` con ``type``/``status`` y
    ``employee_salary_history`` con ``salary_history_id``/``base_salary``, pero
    ``payroll.py`` usa ``leave_type``/``days``/``paid`` y
    ``change_id``/``previous_salary``/``new_salary``; además marca el pago con
    ``payroll_entries.payment_method``, columna inexistente. Renombra y agrega
    columnas de forma idempotente y no destructiva, conservando los datos.
    """
    ...
    _ensure_columns(connection, "employee_time_off", {
...
def _migrate_production_machines_v19(connection: Any) -> None:
    """Crea ``production_machines``, que ``machine_maintenance.py``,
    ``bom_costing.py`` y ``bom_multilevel.py`` dan por existente ("para costeo")
    pero que nunca se creaba en el esquema → "no such table: production_machines".

    Creación idempotente y no destructiva con el contrato que usan esos módulos.
    """
    connection.executescript("""
...
def _migrate_maintenance_usage_columns_v20(connection: Any) -> None:
    """Agrega a ``maintenance_plans`` y ``maintenance_logs`` las columnas de uso
    e inventario que usa ``machine_maintenance.py`` pero que la migración de uso
    (v9) nunca añadió (solo creó ``maintenance_usage_logs``) →
    "no such column: usage_metric". Idempotente y no destructiva.
    """
...
def _migrate_quick_service_prices_v21(connection: Any) -> None:
    """Crea ``quick_service_prices``, que ``quick_sale.py`` usa para la venta
    rápida de mostrador pero que la migración v8 nunca creó (solo agregó una
    columna a ``production_materials``) → "no such table: quick_service_prices".
    Creación idempotente y no destructiva.
    """
    connection.executescript("""
...
def _migrate_missing_tables_v22(connection: Any) -> None:
    """Cierra otros desajustes de esquema respecto al código real:

    - ``exchange_rates``: ``exchange_rates.py`` y ``latest_exchange_rate`` la
      usan pero no existía → "no such table: exchange_rates".
    - ``production_materials.currency``: la usa ``bom_costing.py``.
    - ``session_snapshots``: la v14 la creó con ``user_id``/``snapshot_json``
      NOT NULL, pero ``session_backup.py`` inserta ``data_json``/
      ``sections_included``/``size_bytes`` → el INSERT fallaba. Se reconstruye a
      ese contrato conservando ``snapshot_id``, el JSON y la fecha.
